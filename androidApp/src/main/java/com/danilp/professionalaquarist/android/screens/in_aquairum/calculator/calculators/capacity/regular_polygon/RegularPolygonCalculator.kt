...
@Destination
@Composable
fun RegularPolygonCalculator(
    navigator: DestinationsNavigator,
    viewModel: RegularPolygonCalculatorViewModel = hiltViewModel()
) {
    ...
    Scaffold(
        topBar = {
            AquariumTopBar(
                title = stringResource(R.string.cylinder_title),
        ...
        Column(
            ...
            InfoFieldWithErrorAndIcon(
                value = state.height,
                onValueChange = {
                    viewModel.onEvent(RegularPolygonCalculatorEvent.HeightChanged(it))
                },
            ...
            InfoFieldWithErrorAndIcon(
                value = state.sides,
                onValueChange = {
                    viewModel.onEvent(RegularPolygonCalculatorEvent.SidesChanged(it))
                },
                label = stringResource(R.string.sides_label),
                keyboardOptions = KeyboardOptions(
                    keyboardType = KeyboardType.Number,
                    imeAction = ImeAction.Next
                ),
                ...
                errorCode = state.sidesErrorCode,
                maxLines = 1,
                singleLine = true,
                textFieldModifier = Modifier.fillMaxWidth(),
                metricMeasureCode = state.metricMeasureCode
            ...
            InfoFieldWithErrorAndIcon(
                value = state.length,
                onValueChange = {
                    viewModel.onEvent(RegularPolygonCalculatorEvent.LengthChanged(it))
                },
            ...
            Row(
                ...
                AnimatedVisibility(
                    ...
                    Button(
                        onClick = {
                            viewModel.onEvent(RegularPolygonCalculatorEvent.ApplyButtonPressed)
                        }
                    ) {
                ...
                Button(
                    onClick = {
                        viewModel.onEvent(RegularPolygonCalculatorEvent.CalculateButtonPressed)
                    }
                ) {
