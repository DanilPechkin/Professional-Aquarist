...
@HiltViewModel
class RectangleCalculatorViewModel @Inject constructor(
    ...
    var state by mutableStateOf(RectangleCalculatorState())
    ...
    fun onEvent(event: RectangleCalculatorEvent) {
        when (event) {
            RectangleCalculatorEvent.CalculateButtonPressed -> {
                calculateOutputCapacity()
            }

            is RectangleCalculatorEvent.HeightChanged -> {
                state = state.copy(height = event.height, heightErrorCode = null)
            }

            is RectangleCalculatorEvent.LengthChanged -> {
                state = state.copy(length = event.length, lengthErrorCode = null)
            }

            is RectangleCalculatorEvent.WidthChanged -> {
                state = state.copy(width = event.width, widthErrorCode = null)
            }

            RectangleCalculatorEvent.ApplyButtonPressed -> {
                applyForAquarium()
            }
        }
    }
        ...
        state = state.copy(
            outputCapacity = convertLiters.to(
                ...
                liters = convertLiters.from(
                    ...
                    value = calculateCapacity.rectangle(
