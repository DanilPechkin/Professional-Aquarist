...
@Destination
@Composable
fun CapacityCalculator(
    navigator: DestinationsNavigator,
    viewModel: CapacityCalculatorViewModel = hiltViewModel()
) {
        ...
        Column(
            ...
            Row(
                ...
                InfoFieldWithErrorAndIcon(
                    ...
                    onValueChange = {
                        viewModel.onEvent(CapacityCalculatorEvent.InputCapacityChanged(it))
                    },
                    label = stringResource(R.string.input_capacity_label),
                    ...
                    errorCode = state.inputCapacityErrorCode,
                    maxLines = 1,
                    singleLine = true,
                ...
                OutlinedDropDownMenuField(
                    ...
                    items = state.capacityMeasuresList,
                    selectedItem = state.capacityMeasuresList[state.inputCapacityMeasureCode],
                    changeSelectedItem = {
                        viewModel.onEvent(
                            CapacityCalculatorEvent.InputCapacityMeasureCodeChanged(
                                state.capacityMeasuresList.indexOf(it)
                            )
                        )
                    },
                    modifier = Modifier
            ...
            Row(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { },
                    label = { Text(text = stringResource(R.string.output_capacity_label)) },
                    readOnly = true,
                    modifier = Modifier
                        .padding(end = 16.dp)
                        .weight(2f)
                ...
                OutlinedDropDownMenuField(
                    label = stringResource(R.string.measure_label),
                    items = state.capacityMeasuresList,
                    selectedItem = state.capacityMeasuresList[state.outputCapacityMeasureCode],
                    changeSelectedItem = {
                        viewModel.onEvent(
                            CapacityCalculatorEvent.OutputCapacityMeasureCodeChanged(
                                state.capacityMeasuresList.indexOf(it)
                            )
                        )
                    },
            ...
            Row(
                ...
                Button(
                    onClick = {
                        viewModel.onEvent(CapacityCalculatorEvent.ConvertButtonPressed)
                    }
                ) {
