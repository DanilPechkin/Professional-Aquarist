...
@HiltViewModel
class EllipticalCylinderCalculatorViewModel @Inject constructor(
    ...
    var state by mutableStateOf(EllipticalCylinderCalculatorState())
    ...
    fun onEvent(event: EllipticalCylinderCalculatorEvent) {
        when (event) {
            EllipticalCylinderCalculatorEvent.CalculateButtonPressed -> {
                calculateOutputCapacity()
            }

            is EllipticalCylinderCalculatorEvent.HeightChanged -> {
                state = state.copy(height = event.height, heightErrorCode = null)
            }

            is EllipticalCylinderCalculatorEvent.LengthChanged -> {
                state = state.copy(length = event.length, lengthErrorCode = null)
            }

            is EllipticalCylinderCalculatorEvent.WidthChanged -> {
                state = state.copy(width = event.width, widthErrorCode = null)
            }

            EllipticalCylinderCalculatorEvent.ApplyButtonPressed -> {
                applyForAquarium()
            }
        }
    }
        ...
        state = state.copy(
            outputCapacity = convertLiters.to(
                ...
                liters = convertLiters.from(
                    ...
                    value = calculateCapacity.ellipticalCylinder(
