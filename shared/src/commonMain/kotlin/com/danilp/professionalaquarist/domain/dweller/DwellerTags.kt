package com.danilp.professionalaquarist.domain.dweller

object DwellerTags {
    const val HERBIVOROUS = "Herbivorous" // травоядные
    const val CARNIVOROUS = "Carnivorous" // мясоядные

    const val PREDATOR = "Predator" // хищник
    const val PEACEFUL = "Peaceful" // миролюбивый
    const val TERRITORIAL = "Territorial" // территориальный

    const val VEIL_TAILED = "Veil-tailed" // вуалевый

    const val FAST_CURRENT = "Fast current" // любит быстрое течение
    const val MEDIUM_CURRENT = "Medium current" // любит среднее течение
    const val SLOW_CURRENT = "Slow current" // любит мягкое течение

    const val BRIGHT_LIGHT = "Bright light" // любит яркий свет
    const val LOW_LIGHT = "Low light" // любит слабый свет
    const val IGNORE_LIGHT = "Ignore light" // не обращает внимания на свет

    const val LARGE = "Large" // очень большой
    const val BIG = "Big" // большой
    const val MEDIUM = "Medium" // средний
    const val SMALL = "Small" // маленький

    const val MONOGAMOUS = "Monogamous" // моногамный (пара обязательна)
    const val POLYGAMOUS = "Polygamous" // полигамный

    const val LIVEBEARER = "Livebearer" // живородящие
    const val OVIPAROUS = "Oviparous" // яйцекладущие

    const val SHOAL = "Shoal" // стайный (стая обязательна)

    const val FISH = "Fish" // рыба
    const val SNAIL = "Snail" // улитка
    const val CRAB = "Crab" // краб
    const val SHRIMP = "Shrimp" // креветка
    const val CRAYFISH = "Crayfish" // рак
    const val BIVALVE = "Bivalve" // двухстворчатое

    const val PLANT_LOVER = "Plant lover" // любит растения
    const val NEEDS_SHELTER = "Needs shelter" // нуждается в укрытии

    const val BROADLEAF_PLANT = "Broadleaf plant" // любит широколистные растения
    const val LONG_STEMMED_PLANT = "Long-stemmed plant" // любит длинностебельные растения
    const val FLOATING_PLANT = "Floating plant" // любит плавающие растения
    const val MOSS = "Moss" // мох

    const val CLEANER = "Cleaner" // чистильщик

    const val PLANT_EATER = "Plant eater" // угроза для растений
}